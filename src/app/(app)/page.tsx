...
export default async function MuralPage({
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select(
      "full_name, avatar_emoji, avatar_url, role, rank, stars_total, tribo:tribos!profiles_tribo_id_fkey(id, nome, exercito:exercitos(id, nome))"
    )
    .eq("id", meId)
    .single();
  ...
  const triboAtual = profile?.tribo as unknown as { id: string; nome: string; exercito: { id: string; nome: string } | null } | null;
  let escopoCentral:
  ...
  // Pra filtro "Apenas as que estou envolvido" em CampanhasFeed (pedido do
  // Diretor, 2026-08-27) — Tribo/Exército de quem tá vendo, quando aplicável.
  const meTriboId = triboAtual?.id ?? null;
  const meExercitoId = escopoCentral?.tipo === "exercito" ? escopoCentral.exercitoId : triboAtual?.exercito?.id ?? null;
